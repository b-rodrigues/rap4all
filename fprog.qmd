...
This function makes use of `tryCatch()` which catches any conditions, like warnings, raised by
an expression (in this example, `f(...)`) and then does something. So here I made `tryCatch()` raise an
error (with `stop()`) if the expression raised a warning. It is now possible to define new functions
like this:
...
Because the expression above resulted in an error, `Nothing` is returned. We can then even read the log
...
microseconds to run. But if you’re working with more complex functions, this is a problem. There is
a workaround for this, called *trampolining*. I won’t go into details, but if you’re interested,
there is an R package that allows you to use trampolining with R, aptly called
[`{trampoline}`](https://rdinnager.github.io/trampoline/).
...
This of course depends exactly on what it is your doing. If you writing functions to handle numbers,
that there's little value in placing these numbers inside lists. But in practice, you will very likely
