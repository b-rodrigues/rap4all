...
If there is only one thing that you should remember from this chapter,
it would be pure functions. This is in my opinion not very difficult to
do and comes with many benefits. But, avoiding loops and replacing them
higher-order functions (`apply()`, `Reduce()`,
`purrr::map()` -- and its variants --) also pays off.