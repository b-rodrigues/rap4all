...
```{r}
applying(as.list(seq(1:10)), sqrt)
```
...
In other programming languages, `lapply()` is often called `map()`. The `{purrr}` packages ships with other such useful higher-order
functions that abstract loops away.
