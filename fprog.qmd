...
a very interesting package!). We can then even read the log to see what went
wrong:
...
It is quite common to print the number of steps to the console instead
...
ourselves. This might seem more complicated than repeating yourself, but imagine
...
Up until now I focused on general concepts rather than on specifics of the R
programming language when it comes to functional programming. In this section, we
...
you might want to run some code that creates a plot and saves it to disk, or code
that creates some data and writes them to disk. `local()` can be used for this.
...
so by first creating a temporary file, then downloading it and then loading it.
