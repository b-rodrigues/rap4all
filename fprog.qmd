...
Programming paradagims are ways to structure programs (or scripts).
...
using the [`{memoise}`](https://memoise.r-lib.org/) package. Again, I won't go
into details. So if you want to use and optimize recursive functions, take a
...
other, so we can write a function that does the looping for us. We will write a
function, and call it `looping()`, which will take a function as an argument, as
well as a list. The list will serve as the container to hold our numbers:
