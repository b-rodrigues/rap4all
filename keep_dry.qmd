...
returns literal Markdown code, and we don’t want the parser to have to parse it again.
We tell the parser "don’t worry about this bit of code, it’s already good". As you see, the call
...
Getting tables right in Rmd documents is not always an easy task. There are several packages specifically
...
We've stressed the importance of not repeating oneself by showing you how you could loop
over a function that return literate Markdown code. But sometimes you don't want to have
one section dedicated to each unit of analysis within the same report, but rather, you
want a complete separate report by unit of analysis. This is also possible thanks to
parametrized reports.

Let's modify the example from before, which consisted in created one section per column
of the `mtcars` dataset and create a frequency table, and make it now one separate 
report for each column. The R Markdown file will look like this:
...
It is now possible to knit this document using:
...
`dataset` can be a string here, but needs to be a variable name, so `mtcars` and not "mtcars".
This means that I need to *convert* that string into a name. `get()` searches an object by
name, and then makes it possible to save it to a new variable called `dataset`. The rest
of the function is then the same as before. This little difficulty can be avoided
by hardcoding the dataset inside the R Markdown file, or by passing the dataset as the
`params$dataset` and not the string, in the render function. However, if you pass down the 
name of the dataset instead of the dataset as a string, then you need to covert it to 
string if you want to use it in the text (so `mtcars` to "mtcars").

If you now want to create one report per variable, you could compile all the documents
at once with:
...
try to build your own little parametrized reports. Maybe even try templating inside a parametrized
report! 
