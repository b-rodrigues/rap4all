...
functions with side-effects don’t always return something. Also remember the
example on plotting with `plot()`, which does not return an object. It’s
actually the same issue here.

So to solve this, we need a pure function that returns the `data.frame`. This
means that it first needs to load the data, which results in a promise, and then
evaluate that promise. The function to achieve this is as follows:
...
`{housing}` package and uses the value of `package_name`. Now comes the `envir =` argument. 
This argument tells `data()` where to load the data set. By default, this loads the data
in the global environment. But remember, we want our function to be pure, meaning, it should
only return the data object and not load it into the global environment as well! So
that’s where the temporary environment created in the first line of the body of the function
comes into play. What happens is that the functions loads the data object into this temporary
environment, which is different from the global environment. Once we’re done, we can 
simply discard this environment, and so our global environment stays clean.
...
looks for that name in the temporary environment that was set up. This way, there
is literally no interaction with the global environment, so that function is pure:
it always returns the same output for the same input, and does not pollute, in any way,
...
`{targets}` contributes to the general quality and safety of our pipeline.
...
Remember, the idea is that all the computation is taken out of the `.Rmd` file
...
document. The document simply retrieves the required targets from the cache
and thus gets knitted very quickly. Here is what `analyse_data.Rmd` now looks
like:
...
`renv::restore()` the correct version of the package will get installed!
h 
To finish things, we should edit the `Readme.md` file and add instructions on how
...
`tar_manifest()`
`tar_destroy()`
`tar_visnetwork() %>% visNetwork::visNetworkEditor()`

`withr::with_dir(path, tar_load...)`
