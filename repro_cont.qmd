...
This is the latest `renv.lock` file that we generated for our project, it contains
...
version that was used to originally develop the project, in this case, R version
...
We will use this list to install the required dependencies for our project.
...
this one to restore the project. But it is very likely that I could have
installed the current version of `{renv}` to restore the project, and that it
would have worked without problems. So in the next line I install the right
version of `{renv}`. By the way, I knew how to do this because I read [this
...
contains the required software for our project, but the project files
themselves, to actually run the pipeline, are missing. But before adding the pipeline
itself, let’s see if the Docker image we built is working. For this, login to a command
line inside a running Docker container started from this image with this single command:
...
In summary, the first is approach is "dockerize projects", the second approach
is "dockerize the dev environment and use it for many projects". It all depends
...
To dockerize the project, we first need to understand something important with
...
to add the `_targets.R` script from our project, the `analyse_data.Rmd` markdown
...
That’s it, we have now a complete reproducible analytical pipeline.
