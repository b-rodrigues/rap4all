...
one on the root folder of your project, then that `.gitignore` will work
project-wide.
...
folder containing he sensitive data. Create this file using your favourite text
...
you might be restoring this project and run old versions of packages on a newer
version of R, which may sometimes be a problem (but we’re going to discuss this
later).
...
of R and Rtools and on Windows that should not be too complicated. But that
might be a problem on other operating systems. Does that mean that `{renv}` is
