...
something that we would not want to back up on Github as it grows quite large. So
here is the right moment to introduce the `.gitignore` file (notice the `.` at
the start of the name). This is a file that sits in the root of the directory,
and simply lists files and folder that you want Git to ignore. Any file listed
there will not get pushed to Github. Create this file using your favourite text
editor and simply add the following:

```
renv/
```

This will prevent the `renv/` folder from being tracked and backed up. If you are
working with sensitive, or large, data, you can also add the folder where this data
is to the `.gitignore` file as well.
