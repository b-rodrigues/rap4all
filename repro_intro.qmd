...
reproducible. So we are going to start with the simplest and cost-effective
...
is of writing a package is NOT to upload it to CRAN): packages offer us a great
...
advantage of what packaging our code has to offer, we well learn about the
...
versions of, `{dplyr}` as needed (one per project). The right version of
...
up `{renv}` and run it. You can use `{renv}` once you’re done with an analysis
like in our case, or better yet, immediately at the start of the project. You
can keep updating the `renv.lock` file as you add or remove packages from your
...
and folders that should be ignored. If you open it, you will see that the
`library/` is listed there (among others) so it will be ignored. This is because
it’s the folder that contains all the packages for the project and it might grow
very large, so this is something that we don’t want to track.
...
dependencies, that are used for the project, and it states the R version that
was used when it was generated as well. It is important to remember that when
you’ll use `{renv}` to restore a project’s library on a new machine, the R
...
failed. If that’s the case, let me explain what likely happened.
...
project got removed, or renamed, or simply works differently now. This types
of issues can be avoided by using a per-project library, which is exactly
what `{renv}` does.
...
You can even install arbitrary versions of packages using `renv::install()`. For
example, to install an older version of `{AER}`:
...
reproducibility purposes. Once you’re done with your project, call
`renv::snapshot()` one last time to make sure that every dependency is correctly
accounted for. Don't forget to version the `renv.lock` file using Git!
...
In the next section, I’m going to go over two packages that make it easy to
install old packages, which can be useful for reproducibility as well. But
before that, let’s discuss `{renv}`’s shortcomings (which we already alluded to
before). It is quite important to understand what `{renv}` does and what it
doesn’t do, and why `{renv}` alone is not enough.
...
provide an answer to the question: *how to rerun an old analysis if no renv.lock
file is available?*
...
to know roughly *when* that script was written. Let’s say that you know that
...
[prepint](https://arxiv.org/abs/2303.04758) (@schoch2023).

`{groundhog}` is another option that has been around for more time and is fairly
...
So here again, we are advised to switch to the version of R that was current at
that time. If we follow the message’s advice, and add `tolerate.R.version =
'4.2.2'`, we may get the script to run:
