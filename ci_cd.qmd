...
that event is pushing to the repo.

There are many pre-defined actions that you can choose from to perform a series
of common tasks, you can find them in the 
[Github Actions Marketplace](https://github.com/marketplace)^[https://github.com/marketplace].

We are not going to use any actions from the Github Actions Marketplace, but instead, 
we will be looking at a repository containing actions specifically made for R users.
...
So now my job has two steps, one that prints `"Hello from Bash!"` and another
that prints `"Hello from R!"`. The second step, called `hello-from-R` uses two
actions, `actions/checkout@v3` and `r-lib/actions/setup-r@v2`. The first action,
`actions/checkout@v3` is an action that you will see on almost any Github
Actions workflow file, even though it is likely superfluous in this case. You
can read about it
...
The next action is `setup-r@v2` which downloads and installs, in our example here,
R version 3.5.3. It then runs the command `Rscript -e 'print("Hello from R!")'`. We
could have installed any other version of R by the way. We can keep adding steps, for
example let’s add one to install `{renv}` and install packages from an `renv.lock` file
(the file needs to be in our repository, and becomes available to the workflow thanks
...
`renv.lock` file living in our repository. So to have our RAP running in the
cloud, we would simply need to add the other required files and finish the
...
`ubuntu-latest` and installs the latest version of R. You may want to change
that to the `ubuntu-22.04` (this way, Ubuntu 22.04 will keep getting used even
when the next LTS, 24.04, will be released) and install R version 4.2.2 (or
whichever version you used for your pipeline). This workflow file also runs some
other useful actions, like caching packages, so they don’t need to get
