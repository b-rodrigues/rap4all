...
automate them and only take care of writing code? And the size of your team
grows, the need to automate these tasks grows as well.
...
- build a Docker image containing a development envirornment and push it to Docker Hub when pushing changes to its Dockerfile on Github;
...
this line into the yaml file, put the two lines back into one line.  Click
[here](https://raw.githubusercontent.com/b-rodrigues/ga_demo/6b7bba10b9ce8efc9c4877bb4908d742b1663a7c/.github/workflows/build-renv.yml)^[https://is.gd/0xqH22]
for the actual file.)
...
(remember, to write a command over multiple lines on github actions, you need to
...
`tarets::tar_github_actions()` generates. But I think that this solution is
...
At the start of this chapter, I stated that this chapter was optional, because it
is not necessary to use a CI/CD service to ensure that your projects are
...
to our code, for very little gain. In some ways, forcing yourself to use a CI/CD
service can feel similar. But here’s the thing: if your project builds
successfully on a CI/CD service, and if the results remain stable through time,
then your project is reproducible. Someone else could then run it locally by
simply following the same steps as in the workflow file, which would consist in
