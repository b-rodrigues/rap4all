...
`main` or `master`. We define an environment variable called `4_2_2`. This is
...
build RAPs. Then we use the `docker/login-action@v2` to login from Github
Actions to Docker Hub. Essentially, we need to be able to tell our Github
Actions runner how to login to Docker Hub, and of course we want to do so in a
secure manner (and it must run non-interactively). Then, the action
`docker/setup-buildx-action@v2` and as its name indicates, this simply sets up
everything for `buildx` to run smoothly (`buildx` will build the Docker image
using the `docker buildx` command an alternative to `docker build`).
