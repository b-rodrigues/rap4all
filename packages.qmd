...
larger and larger, we realise that we might need something else than a simple
`.Rmd` file.
...
`{fusen}`’s author, a reality. The idea is to start from documentation in the form
of an `.Rmd` file and go from there to a package. Let’s dive right into it by
starting from a template included in the `{fusen}` package. Start an R session
...
`dev/`. Let’s see what’s inside (I use the command line to list the files, but
you’re free to use your file explorer program):
...
second file `flat_minimal.Rmd` (or our very own `.Rmd` files) contain functions,
...
tests, or worse, instead of testing your functions on your console only, and
...
the same with `Imports:`, and the difference with `Depends:` is most of the time
irrelevant: packages listed under `Depends` will not only be loaded when you
load your package, but also attached. This means that the functions from these
packages will also be available to the end-user when loading your package.
Packages listed under `Imports:` will only be loaded, meaning that their
functions will only be available to your packages’ functions, not the end-users
themselves. If that’s confusing, don’t worry too much about it. Finally,
`Suggests:` are dependencies that are not critical, usually these are only
necessary if you want to run the code from the packages vignettes or examples.
As you can imagine, listing the right packages under the right category can be a
daunting task. But don’t worry, `{fusen}` takes care of this automatically for
us! Simply focus on writing your `.Rmd` files.
...
do, use GPL’ed code in their internal products. 
...
`docs/` folder in the root of your package folder. This can then be hosted, for
free, with Github so people can explore the package’s functions and
...
Because we already have a folder for our project, called `housing/` we use the
...
`dev/` folder. Move `save_data.Rmd` inside; remember, we only want to inflate
...
like](https://is.gd/anRjt4) (no worries, I’m going to explain how I got there).
...
and [after](https://is.gd/anRjt4), the differences are that we have named the
chunk `function-get_raw_data` and added documentation in the form of
`{roxygen2}` comments. Naming the chunks is essential: this is how `{fusen}`
knows that this chunk contains a function that should go into the `R/` folder.
`{roxygen2}` are strictly speaking not required, but it is highly advised that
you add them: this way, your function will get documented and users (including
future you) will be able to read the documentation by typing
`help(get_raw_data)`. Another difference is that I have made all the functions
referentially transparent. Take a closer look at `make_plot()` in the before and
after `.Rmd`’s. You will see that I’ve added two arguments to `make_plot()`,
`country_level_data` and `commune_level_data`. This is really important, so
...
functions decorated with the `@export` keyword are public, functions without it
are private. But private functions don’t really exist in R. You can always
access a "private" function by using `:::` (three times the `:`), as in
...
RAP, and don’t want to have to look at it as is running to see if everything is
...
of future you to have to manually check console output or logs and look for
...
this example to run successfully, I would need to redefine `commune_level_data`
from scratch. I don’t want to do this now, so hence why I wrapped the example
around `\dontrun` and used roxygen-style comments with `#'`. I did the same with
...
`save_data.Rmd`! Let’s not forget that the whole point of `save_data.Rmd` file was,
...
`usethis::use_data()`, mimicking the steps above. I wrapped all the code around
the `local()` function to run all these steps inside a temporary, local
environment. This way, any variable that gets made by knitting the vignette gets
discarded once we’re done.
...
we’re going to discover the following two scenarios: the package is hosted on
...
Once your package is done, whether it is destined for CRAN or not, whether it can 
...
and thankfully, it but takes two lines of code to build a fully functioning site.
In the introduction we built the website for the template included with `{fusen}`,
let’s now build a website for our housing package.
...
pages. For this, first make sure that the `.gitignore` file in the root of your
package does not contain the `docs/` folder. If yes remove it. Then, commit and
...
especially if you want your users or future you to install the package at a certain
commit, it is quite useful to write it down clearly in the instructions. Something
like:
