...
project will stay with us until the end of the book. As we will go deeper in the
book together, you will rewrite that project by implementing the techniques I
will teach you. By the end of the book you will have built a reproducible
...
explain what these packages do, in case you're not familiar with them. The `{dplyr}`
package provides many functions for data manipulation, for example aggregating
group-wise. `{purrr}` is a package for functional programming, a programming
paradigm that I'll introduce later in the book, `{readxl}` reads in Excel
workbooks, `{stringr}` is a package for manipulating strings, and finally
`{janitor}` provides some very nice functions, to perform some common tasks like
easily rename every column of a data frame in snake case.
...
eventually catch as you’ll explore the data: naming of the communes is
...
commune. We could keep that in a separate data frame, or even remove it. Finally
there's a row, the last one, that states the source of the data, which we can
remove.
...
list.](https://en.wikipedia.org/wiki/List_of_communes_of_Luxembourg)^[https://en.wikipedia.org/wiki/List_of_communes_of_Luxembourg]
...
since we only have one 'good' (housing), this index becomes quite simple to compute: it is
...
actually run the analysis proper. Performing the analysis might seem quite easy,
because each script contains comments as to what is going on, and the code is
not that complicated. However, we are missing any project-level documentation
...
course, should the analysis be more complex (suppose it’s composed of a dozens
...
writing functions, even when simply to reuse them inside the same project.
