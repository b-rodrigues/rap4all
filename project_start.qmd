...
an analysis done. We are going to download some data, analyse it, and write a little report. Then,
...
Markdown source file that outputs the finalised document in one command line.
...
You can click [here]("rmds/save_data.Rmd") to take a look at the finalised Rmd file. The advantage
...
This Rmd file makes use of the `knitr::knit_child()` to knit a child document in order to create
sections for selected communes without needed to copy and paste code.

Now, you would very likely stop at this point: after all, we are done with the analysis. However, 
...
- What guarantee do we have that if the documented gets knitted in 5 years, with the same input data, we get the same output?
...
The analysis is composed of two R Markdown files, one to prepare the data, another to actually run the analysis
proper. This might seem quite easy, because each Rmd file contains many explanations as to what is going on.
However, we are missing any project-level documentation, that would give clear instructions. It might seem obvious
to you, the author of this analysis, that first the file `save_data.Rmd` should be knitted, and then the file
called `analyse_data.Rmd`, but that’s not necessarily the case for someone completely outside of the project.
It might not even be obvious to you in 5 months! Also, you are assuming that the person that should rerun this
project is familiar with R and knitting Rmd files. That’s not necessarily the case. 

And what about the required dependencies? Many packages were used in the analysis. How should these get installed?
Ideally, the same versions of the packages you used and the same version of R should get used by that person
...
If new data gets published, all the points discussed previously are still valid, plus you need to make sure that
the updated data is still close enough to the previous data that it can pass through the data cleaning steps
you wrote. You should also make sure that the update did not introduce a mistake in past data, or at least
alert you if that is the case. Sometimes, when new years get added, data for previous years also get corrected,
so it would be nice to make sure that you know this. Also, in the specific case of our data, communes might
get fused into a new one, or maybe even divided into smaller communes (even though this is has not happened
in a long time, it is not entirely out of the question).

In summary, what is missing from the current project are enough tests to make sure that an update to the data
can happen smoothly.

### What guarantee do we have that the output is stable?

Now this might seem weird: after all, if we start from the same dataset, does it matter *when* we knit the document?
We should be getting the same result if we build the project today, in 5 months or in 5 years. Well, not necessarily.
While it is true that R is quite stable, this cannot necessarily be said of the packages that get used. There is
no guarantee that the authors of the packages will not change the package’s functions to work differently, or 
take arguments in a different order, or even that the packages will all be available at all in 5 years.
And even if the packages are still available and function the same, bugs in the packages might get corrected that
could now alter the result. True, your results depended on a bug and now that the bug is corrected your results
should be "correct" as well, but if you need to reproduce the results as-is, mistakes and all, this could actually
be problematic.
