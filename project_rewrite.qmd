...
using the chunk option `eval = F`, like I did, you could use `include = F`, so
it doesn’t appear in the compiled document at all. The first time you compile
this document, you could change the option to `eval = T`, so that the data gets
written to disk, and then change it to `eval = F` to avoid overwriting the data
...
argument has a default value of "2010". This is the year the data starts, and
so this the year the Laspeyeres index will have a value of 100.
...
called `locality` in body of the function, but since there is no variable
...
tell this to our function: *don’t worry about evaluating this just yet, I’ll tell
you when it’s time*.
...
your needs, and you could of course write many more details that me. I wanted to
keep it short, the point of this chapter was to show you how to use literate
programming not, to write a very detailed analysis.
...
a couple of years (but be careful with this assessment, sometimes an analysis
that wasn’t supposed to be reproducible turns out to be reproducible for way
